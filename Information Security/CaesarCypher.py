...
if choice == "1":
    ...
    for c in input_message:
        if (65 <= ord(c) <= 90):
            encrypted_message += chr(65 + (alphabetTable[c] + key) % 26)
        elif (97 <= ord(c) <= 122):
            temp = ord(c) - 32
            encrypted_message += chr(97 + (alphabetTable[chr(temp)] + key) % 26)
        else:
            encrypted_message += c
    ...
    for c in input_message:
        ...
        if (65 <= ord(c) <= 90):
            decrypted_message += chr(65 + (alphabetTable[c] - key) % 26)
        elif (97 <= ord(c) <= 122):
            temp = ord(c) - 32
            decrypted_message += chr(97 + (alphabetTable[chr(temp)] - key) % 26)
        else:
            decrypted_message += c
